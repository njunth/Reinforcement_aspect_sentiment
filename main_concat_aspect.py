...
def line_to_tensor(label_indicat, words, model, padding=False):
...
def pre_train():
    ...
    l_indicat = label_indicator(train_x_text)

    train_y_tensor = [torch.LongTensor([i]) for i in train_y]
    x_feature_train = [line_to_tensor(l_indicat, s.split(), w2v_model) for s in train_x_text] #N*1*dim
    x_feature_dev = [line_to_tensor(l_indicat, s.split(), w2v_model) for s in dev_x_text]

    ## PreCNet
    
    # PreCNet = TextRNN(300, 128, 2)
    PreCNet = TextRNN( 301, 128, 3 )
    acc = 0
    print("Pre-training CNet")
    n = 0
    while(acc < 0.78): # For a good pre-train CNet.
        acc = train(x_feature_train, train_y_tensor, x_feature_dev, dev_y, PreCNet, epochs=1, savename="PreCNet")
    ...
    # PrePNet = ID_LSTM(300, 128, 2)
    PrePNet = ID_LSTM( 301, 128, 3 )
...
def test(pretrained_model):  # load a model and test on a sentence.
    ...
    sentence = "HARDWARE GENERAL No hardware issues, as is common with any Apple product."
    sentence_1 = "COMPANY GENERAL No hardware issues, as is common with any Apple product."
    ...
    x_text_list = clean_str( sentence_1 ).split()
    x_feature_tensor = line_to_tensor( x_text_list, w2v_model )
    test_model.eval()
    output = test_model( Variable( x_feature_tensor ) )
    print(x_text_list)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1: # FOr IDE Running.
        mode = "joint-train"
        pretrained_model_name = "backup_saved/" + "PrePNet_0.777.pt"
    if len(sys.argv) > 1:
        mode = sys.argv[1]
    if len(sys.argv) > 2:
